...
const getClient = () => {
  ...
  if (!apiKey) {
    throw new ApiError(503, "Missing Gemini API Key in the .env file.");
  }
...
const generateJSON = async (prompt, schema) => {
  ...
  try {
    ...
    console.log("Gemini JSON Error: ", err?.message || err);
    console.error(err);
    throw err;
    throw new ApiError(502, "AI request failed. Try again after sometime.");
  }
};
...
export const generateLeadSummary = async (lead) => {
  const prompt = `You are an expert B2B sales analyst for a CRM called ${process.env.APP_NAME}
  ...
  const schema = {
    ...
    properties: {
      ...
      riskScore: {
        type: "integer",
        description: "Risk of losing this deal, 0 (safe) to 100 (high rish)",
      },
      suggestedPriority: {
        type: "string",
...
export const generateEmail = async ({ lead, purpose, tone, sender }) => {
  const prompt = `You are a senior Sales Representative writing on behalf of
    ...
    Write a professional sales E-Mail.
...
export const generateSalesInsights = (pipelineInsights) => {
  const prompt = `You are a revenue-operations advisor. Given this snapshot of a sales pipeline,
    identify what is working, what is at risk, and concrete actions to improve conversion.

    Pipeline Snapshot (JSON):
    ${JSON.stringify(pipelineInsights, null, 2)}
  ...
  const schema = {
    ...
    properties: {
      ...
      insights: {
        type: "array",
        description: "3-5 specific, data-driven operations.",
        items: { type: "string" },
      },
      recommendations: {
        type: "array",
